...
export function ChildHome() {
  ...
  const goodMomentsToday = state.logs.filter(
    (l) => l.logDate === today && l.outcome !== 'not_yet',
  );
              ...
              {goodMomentsToday.map((log, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-base font-medium">{item?.title ?? 'Một việc tốt'}</span>
                  </li>
                );
              })}
            </ul>
        ...
        <GardenCard>
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-sm font-medium text-leaf-600">
              {activeRewards.filter((r) => canRedeem(state.ledger, r.pointsCost).affordable).length}{' '}
              đã nở 🎁
            </span>
          </div>
            ...
            {activeRewards.map((reward) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm font-semibold leading-snug text-leaf-800">{reward.title}</p>
