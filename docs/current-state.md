...
- **Situation library — parent coaching (non-AI; product-spec §16.2, first depth feature past the checklist):** gated route `/parent/situations` ("Cẩm nang tình huống", linked from the dashboard). An editorially-written, static handbook (`src/lib/mock/situations.ts`, 8 seed scenarios — anger/tantrum, lying, school fear, toy grabbed, won't stop playing, losing a game, "con ghét bố/mẹ", sibling fights) structured on the Phan Hồ Điệp 4-step (acknowledge → calm → empathize → resolve) + a "tránh điều này", each tagged with age bands + an optional virtue. Pure `situationsForBand` filter + fixed `SITUATION_STEPS` (`src/lib/domain/situations.ts`); the route defaults to the child's runtime band with an "all bands" toggle and an accordion per scenario. No AI, no backend, no diagnosis, parent-mediated — this is the buildable-now seed of the premium AI Parenting Coach. New domain type `SituationGuide`. Tested (`situations.test.ts` content + filter; `ParentSituations.test.tsx` band filter + expand).
...
- **Product direction decided 2026-06-16 (depth & lifecycle — see product-spec.md §16, feature-ideas #11/#12):** (1) the checklist is the floor, not the product — daily life must come from layers *around* it (emotion check-in, "việc của tuần", calm corner, living garden), never by churning the list; (2) the **parent-coaching layer is the paid value** — a non-AI **situation library** (Phan Hồ Điệp 4-step) is buildable now and seeds the premium **AI Parenting Coach**; (3) **lifecycle to 16** — add teen bands `11-13`/`14-16` and **retire child mode at puberty** (proposed cutoff 11), leaving a parent-only coaching companion. All three are post-MVP and deliberately pull ambition forward; the non-AI pieces (situation library, variety layers) are built first since they don't wait on the Supabase swap. Re-prioritized in feature-ideas-eq.md.
...
- **Puberty cutoff for retiring child mode** (product-spec §16.3): exact age (proposed 11) and how the dual → parent-only transition is communicated. Needed before building the lifecycle-to-16 extension.
